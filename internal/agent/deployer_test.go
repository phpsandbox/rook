...
func TestDeployerRedeployCutsOverAfterNewContainerIsHealthy(t *testing.T) {
	...
	if !slices.ContainsFunc(docker.events, func(event string) bool {
		return strings.HasPrefix(event, "build:deployment-deploy-1:")
	}) {
		t.Fatalf("deployment image tag missing from events: %#v", docker.events)
	}
}
...
func TestDeployerPreparesBundleBuildContextWithoutOverlayingSource(t *testing.T) {
	...
	payload := DeployPayload{
		...
		Bundle: &bundle,
		Env:    map[string]string{"COMPOSER_AUTH": "secret"},
	}
	...
	if docker.buildEnv["COMPOSER_AUTH"] != "secret" {
		t.Fatalf("build environment = %#v", docker.buildEnv)
	}
