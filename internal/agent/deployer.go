...
func (d *Deployer) Deploy(ctx context.Context, payload DeployPayload, send func(OutboundMessage)) error {
	...
	imageTag := fmt.Sprintf("deployment-%s:%s", payload.DeploymentID, time.Now().Format("20060102-150405"))
	emitLog("build", "Building Docker image...")
	if err := d.docker.Build(ctx, buildContextDir, imageTag, payload.Env, func(line string) {
		emitLog("build", line)
	}); err != nil {
